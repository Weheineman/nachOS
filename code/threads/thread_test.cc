...
void PortTestSender (void* structPointer_){
    ...
    printf("%s successfully sent a message.\n", currentThread ->GetName());
...
void PortTestReceiver (void* structPointer_){
    ...
    printf("%s successfully received a message.\n", currentThread ->GetName());
...
void LateJoinTest(void *finishCheck_){
	Semaphore* finishCheck = (Semaphore *) finishCheck_;
...
ThreadTest()
{
    ...
    #elif defined LATE_JOIN_TEST
    Thread ** sons = new Thread*[threadAmount];
    Semaphore* finishCheck = new Semaphore("finishCheckSemaphore", 0);
    ...
    for(int threadNum = 1; threadNum <= threadAmount; threadNum++){
        ...
        #if defined(JOIN_TEST) || defined(LATE_JOIN_TEST)
        ...
        sons[threadNum - 1] = newThread;
        newThread->Fork(LateJoinTest, (void *) finishCheck);
